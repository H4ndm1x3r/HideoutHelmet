...
function readADXL335() {
    ...
    if (xAxis < 0.98 || xAxis > 1.2) {
        clearInterval(gestureMathDown);
    }
    else {setInterval(gestureMathDown,200);}
    // round y-axis to 2 decimals.
    yAxis = Math.abs(Math.round(d[1] * 100) / 100);
    if (xAxis < 0.4 || xAxis > 0.6) {
        clearInterval(gestureMathSide);
    }
    else {setInterval(gestureMathSide, 200);}
    // round z-axis to 2 decimals.
    zAxis = Math.round(d[2] * 100) / 100;
    if (zAxis < 0.6 || zAxis > 0.8){
        clearInterval(gestureMathUp);
    }
    else {setInterval(gestureMathUp,200);}
}
//----------------------------------------------------------------\\
...
function gestureMathDown() { // decide whether gesture is head-down based on x-axis
    ...
    if ( /* adjust these variables and the following higher for more agression*/ xVariable1 >= 0.98 && xVariable1 <= 1.2) { // if the first x-variable is within the set range, set the second.
        xVariable2 = xAxis;
    }
    if (xVariable1 > 0.98 && xVariable1 < 1.2 && xVariable2 > 0.95) { // if both first and second x-variables meet the requirements, set the third
        var xVariable3 = xAxis;
        xVariable = xVariable1 + xVariable2 + xVariable3;
    }
    console.log(xAxis); // log for troubleshooting down movement
    if (xVariable < 3.01 || xVariable > 3.2 && xMoved === true) { // initial test for downmovement
        findMovementX = !findMovementX;
        xMoved = !xMoved;
    }
    if (findMovementX == 1 && xVariable >= 3.01 && xVariable <= 3.20 && xMoved === false) { // allow the down motion
        xMoved = true;
        //console.log("Moved all they way down");                   // log for troubleshooting if-statements
        return xMoved;
    }
    if (findMovementX == 1 && xMoved === true) { // perform the down motion
        s.move(0, 500);
...
function gestureMathUp() { // decide whether gesture is head-up based on z-axis
    ...
    if (findMovementZ == 1 && zVariable >= 1.26 && zVariable <= 1.60 && zMoved === false) { // allow back mtion
        zMoved = !zMoved;
    }
    if (findMovementZ == 1 && zMoved === true) { // perform back motion
        s.move(1, 500);
        console.log("up");
    }
